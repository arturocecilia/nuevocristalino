...
?>
   ...
   <div class="archive-lente-wrapper">
	<div id="post-<?php the_ID(); ?>" <?php post_class('iolCrossLang'); /*al final hemos puesto esta clase porque el estilo era mejor */ ?>>	
     ...
     <?php if ( is_sticky() && is_home() && ! is_paged() ) : ?>
		...
		<div class="entry-content iol-entry-content">
            <h1 class="archive-iol-title">
				<a href="<?php the_permalink(); ?><?php echo $pt;?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'twentytwelve' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark">
	...
	if( (isset($_COOKIE["ncpatient"]) || isset($_GET["nivel-pref-lente"]))  && (get_post_meta($post->ID, 'simpleLensDesc', TRUE) != '//') && (get_post_meta($post->ID, 'simpleLensDesc', TRUE) != '')){
			...
			if ($fabricante = get_the_term_list( $post->ID, _x('fabricante-lente','taxo-name','iol'))){
				echo'
					<div class="bloq-single-lente">
						<div class="label">'._x("Fabricante de la Lente:",'Content Archive Lente Intraocular Paciente',"iol_theme").'</span></div>
         ...
         <div class="iol-entry-meta">
           ...
           <a href="<?php the_permalink(); ?><?php echo $pt;?>"><?php echo _x("Ver lente","Content Archive Lente Intraocular","iol_theme"); ?></a>
