...
function addChangeModeBloqContent() {
  jQuery.ajax({
    ...
    success: function(response_from_the_action_function) {
      jQuery("#changeModeBloq div.contenidoMode").html(
...
function Ajaxor() {
  if (getPartOfTheSite()) {
    if (typeof history.pushState !== "undefined") {
      ...
      jQuery(
        "#destacados .destacado1 h2 a, #mVisitedIols a:not(#buscadorBody footer#colophon a), #corporativeLinks a:not(#buscadorBody footer#colophon a,body.home a), div.link-to-archive a:not(#buscadorBody footer#colophon a), .content-destacado a, .block-last h2 a, .block h2 a, block-last h2 a:not(#buscadorBody footer#colophon a)"
      ).live("click", function() {
        var href = jQuery(this).attr("href");
...
function goToContent(href, hashScrollTo) {
  ...
  jQuery.ajax({
    ...
    success: function(data) {
      ...
      //De la función respuesta tambien cogeremos #main -> En un segundo paso esto no será necesario puesto que será lo único que devolvamos con ajax.
      console.log("Petición Ajax realizada con éxito");
      var mainContent = jQuery(data)
        .find("#content")
        .html(); //Queremos sólo el interior de #content
...
// if (jQuery('#countrySitesList').css('opacity') == '0'){
             jQuery('#top-header-countries').animate({opacity:1}, {duration: 700});//css('display', 'block');
/*          }
          else
           {*/
...
function hideSites() {
            jQuery('#top-header-countries').animate({opacity:0}, {duration: 700});
}
...
function toggleSites() {
 ...
 if (jQuery('#top-header-countries').css('opacity') == '0'){//#countrySitesList
 			 jQuery('#top-header-countries').css('display','block');//#countrySitesList
             jQuery('#top-header-countries').animate({opacity:1}, {duration: 300});//css('display', 'block');
