...
if (!(! empty($_SERVER[ 'HTTP_X_REQUESTED_WITH' ]) && strtolower($_SERVER[ 'HTTP_X_REQUESTED_WITH' ]) == 'xmlhttprequest')) {
    ...
    function hrefLangGenerator($id_MainQ)
    {
        ...
        $getlocaleIni = get_locale();

        //Tenemos 3 idiomas: Español (site:1 ), Inglés (site:3), Alemán (site:4)

        //Tenemos un dominio por cada site.
        $ncWPSitesDomains= array(
                'es-ES'     =>      'https://www.nuevocristalino.es',
                'es-MX'     =>      'https://www.nuevocristalino.mx',
                'en-GB'     =>      'https://www.newlens.co.uk',
                'de-DE'     =>      'https://www.neuelinsen.com',
                'es-CO'     =>      'https://www.nuevocristalino.co',
                'fr-FR'     =>      'https://www.nouveaucristallin.com',
                'es-CL'     =>      'https://www.nuevocristalino.cl',
                'de-AT'     =>      'https://www.neuelinsen.at',
                'en-US'     =>      'https://www.mylifestylelens.com');

        //Cada uno de los dominios anteriores tiene un añadido correpondiente al recurso solicitado sólo en función del idioma => Las 2 primeras letras .
        $ncWPLangs = array(
                        'es'     =>      1,
                        'en'     =>      3,
                        'de'     =>      4,
        ...
        $ncWPLangsDomains = array(
                        'es'     =>      'https://www.nuevocristalino.es',
                        'en'     =>      'https://www.newlens.co.uk',
                        'de'     =>      'https://www.neuelinsen.com',
                        'fr'		 =>			 'https://www.nouveaucristallin.com'
                    );


        //Definimos el array que tendrá la url junto con los idiomas
        $ncWPLangsURL = array();
        ...
        if (is_home()) {
            foreach ($ncWPSitesDomains as $key=>$value) {
                echo '<!-- Optimizado -->';
        ...
        if (is_page()) {
            echo '<!-- Optimizándose -->';
            foreach ($ncWPLangs as $key=>$value) {
                switch_to_blog($value);



                //INI: Adoption of new sistem for sync
                $relatedLang = $ncWPLangLocal[$key];
                $idRelatedObject = $capsule->table('nc_sync')->where($getlocaleIni, $id_MainQ)->first()->{$relatedLang};
                //END: Adoption of new sistem for sync


                $url = get_permalink($idRelatedObject);

                $ncWPLangsURL[$key] = str_replace($ncWPLangsDomains[$key], "", $url);

                restore_current_blog();
            }

            foreach ($ncWPSites as $key=>$value) {
                if (get_post_meta($id_MainQ, $key, true) == 'NA') {
                    $mediumDash = str_replace('_', '-', $key);
                    unset($ncWPSitesDomains[$mediumDash]);
                }
            }

            foreach ($ncWPSitesDomains as $key=>$value) {
                $langDomain = mb_substr($key, 0, 2);
                echo "<link rel=\"alternate\" hreflang=\"".$key."\" href=\"". $value . $ncWPLangsURL[$langDomain]."\" /> ";
            }
        ...
        if (is_single() && ($post_typeT == _x('lente-intraocular', 'CustomPostType Name', 'iol') || $post_typeT == _x('fabricante', 'CustomPostType Name', 'fabricante') || $post_typeT == 'post')) {
            if ($post_typeT == _x('lente-intraocular', 'CustomPostType Name', 'iol')) {
                foreach ($ncWPLangs as $key=>$value) {
                    ...
                    $rawUrl = get_permalink($idRelatedObject);//$id_MainQ);
                    ...
                    //le quitamos el domain y cargamos el añadido de la url en el array de urls.


                    $ncWPLangsURL[$key] = str_replace($ncWPLangsDomains[$key], "", $iolLink);
            ...
            if ($post_typeT == _x('fabricante', 'CustomPostType Name', 'fabricante')) {
                foreach ($ncWPLangs as $key=>$value) {
                    ...
                    $ncWPLangsURL[$key] = str_replace($ncWPLangsDomains[$key], "", $fabricanteLink);
            ...
            //Queremos meter la posibilidad de que el post sólo esté en una de las versiones.
            //Habrá dos niveles de restricciones:
            //Qué solo esté en los sites de un determinado idioma.
            //Qué solo esté an algunos sites independientemnete del idioma.
            /*
            La mejor manera de hacer lo anterior es actuar directamente sobre los

            ncWPLangsURL        => Array generado dinámicamente donde se carga la el añadidio a la home url correpondiente al recurso en cada idioma.
            ncWPLangs           => Tiene como key las dos primeras letras de la lang y como value el id del site
            ncWPLangsDomains    => Tiene sólo las home_url de los sites de referencia de cada idioma.
            ncWPSitesDomains    => Tiene el código de lengua y luego el  site url correspondiente.

            ---
            si cambias ncWPLangs modificas la creación de los links relativos a cada idioma.
            si cambias ncWPSitesDomains alteras la impresión de los hreflangs
            --
            No quiero añadir más procesamiento.

            */

            if (($post_typeT == 'post')) {
                //Tengo que buscar donde están definidos los metaboxes de los idiomas y cargármelos. Sustituirlos por un sólo input y que si tiene valor nos saque de este if.

                //Si el post es en frances


                //Definimos un array con los ids de los posts que no se propagan estándard=> que tienen distintos contenidos/títulos dentro del mismo idioma.
    $arraySpecificPosts = array(13595,13867);//9153, 9169
    $checkSpecific = in_array($id_MainQ, $arraySpecificPosts);
                ...
                if (!$checkSpecific) {
                    foreach ($ncWPLangs as $key=>$value) {
                        ...
                        $postLink =  get_permalink($idRelatedObject);//$id_MainQ) ;
                        $ncWPLangsURL[$key] = str_replace($ncWPLangsDomains[$key], "", $postLink);

                        restore_current_blog();
                    }


                    //As for the time being we do not have post translated to French we simply remove the french site from the array of sites,




                    //Vammos a hacer un par de unsets en función de los metadatos builtin: built-in-fr_FR
                    /*
                    if(get_post_meta($id_MainQ,'built-in-en_GB',true) == 0){
                            unset($ncWPSitesDomains['en-GB']);
                        }*/


                    unset($ncWPSitesDomains['fr-FR']);

                    //unset($ncWPSitesDomains[str_replace('_','-',get_locale())]);
           ...
           //Seguimos excluyendo los posts en Francés de la generación de hrefs porque todavía no hay.
                    unset($ncWPSitesDomains['fr-FR']);

                    //Vemos si del post actual hay NA como metadato de no contenido análogo. De ser así lo quitamos del array ncWPSitesDomains.
                    foreach ($ncWPSites as $key=>$value) {
                        if (get_post_meta($id_MainQ, $key, true) == 'NA') {
                            $mediumDash = str_replace('_', '-', $key);
                            unset($ncWPSitesDomains[$mediumDash]);
                    ...
                    //Desde el ncWPSitesDomains ya limpiado. Lo recorremos sacamos el número de blog correspondiente mediante la llave adaptada puesta en ncWPSites.
                    //hacemos el switch_to_blog correspondiente y ponemos el permalink correspondiente.
                    foreach ($ncWPSitesDomains as $key=>$value) {
                        $modKeyToNumber = $ncWPSites[str_replace('-', '_', $key)];
                        switch_to_blog($modKeyToNumber);
                        echo '<link rel="alternate" hreflang="'.$key.'" href="'. get_permalink($id_MainQ).'" /> ';
                        restore_current_blog();
                    }
                }
            }
    ...
    /* PARSEANDO LA URL----> */
                ...
                if (is_archive()) {
                    ...
                    foreach ($ncWPLangs as $key=>$value) {
                        ...
                        if (strpos($taxoNameLoca, 'intraocular-lens-type')!== false) {
                            $taxoNameLoca = str_replace("intraocular-lens-type", "lens-type", $taxoNameLoca);
                        }
                        ...
                        if ($doHrefLangInTaxo  && !$paginada) { //is_wp_error( $taxoTermLink ) //el get_term_link no estaba localizando "lente-intraocular"
                            ...
                            $taxonLink = $rootCurrentUrlSite.'/'.$slugIOL.'/'.$taxoNameLoca.'/'.$termNameLoca;

                            $ncWPLangsURL[$key] = str_replace($ncWPLangsDomains[$key], "", $taxonLink);
        ...
        //Sacamos el término localizado.
                            ...
                            if ($doHrefLangInTaxo  && !$paginada) { //is_wp_error( $taxoTermLink ) //el get_term_link no estaba localizando "lente-intraocular"
                                ...
                                $taxoBisLink = $rootUrl.'/'.$urlOnlyTaxoSlugEndLoca.'/'.$termNameLoca;
                                $ncWPLangsURL[$key] = str_replace($ncWPLangsDomains[$key], "", $taxoBisLink);
