<?php
/*
 * Template Name: Template UDA Display Prof Result
...
if($current_user_ai->ID){
							...
							if($checkProf != 'prof'){
								echo '<p>'._x('Usted no está logeado como usuario profesional, por favor proceda a editar su:','template-uda-displayprofresults','iol_last').' <a href="'.get_permalink(12644).'">'._x('información básica','template-uda-displayprofresults','iol_last').'</a> '._x('o póngase en contacto con nosotros','template-uda-displayprofresults','iol_last').'</p>';
								}else{
					...
					if(count($uProfClinics) == 0){
											echo '<br />';
											echo '<p>'._x('Estimado usuario:','template-uda-displayprofresults','iol_last').' <strong>'.$uName.'</strong> '._x('según la info aportada, usted no tiene permiso para ver resultados específicos de una clínica.','template-uda-displayprofresults','iol_last').'</p>';
							}else{
											echo '<br />';
											echo '<p>'._x('Estimado usuario:','template-uda-displayprofresults','iol_last').' <strong>'.$uName.'</strong> '._x('según la info aportada, puedes ver los datos asociados a las siguientes clínicas:','template-uda-displayprofresults','iol_last').'';
                      echo '<div class="clinicResultListWrapper">';
											foreach($uProfClinics as $idClinic){
                        //get_permalink()
                          echo '<form method="post" action="'.get_permalink(15228).'">';
                      ...
                      echo '<p>'._x('En el siguiente link podrá ver los resultados de los pacientes que han sido operados por usted:','template-uda-displayprofresults','iol_last').'</p>';
                      echo '<div class="clinicResultListWrapper">';
                        //get_the_ID() 15228
                          echo '<form method="post" action="'.get_permalink(15228).'">';
                          echo '<input type="hidden" name="sxs_id" value="'.$current_user->ID.'" />';
                          echo '<div class="linkToClinicResults"><input type="submit" value="'._x('Info rellenada por sus pacientes','template-uda-displayprofresults','iol_last').'" /></div>';
                          echo '</form>';
                      echo '</div>';
                    }
	}
            ...
            echo '<br /><br /><p>'._x('Recuerda que has de estar registrado y haber iniciado sesión para poder ver los niveles de satisfacción de tu operación en otros usuarios.','template-uda-displayprofresults','iol_last').'</p>';
          }
