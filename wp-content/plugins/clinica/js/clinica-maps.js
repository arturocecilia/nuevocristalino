...
if (site.Country) {
  ...
  switch (site.Country) {
    ...
    case "es_CL":
      var referenceLatitude = -33.46912;
...
jQuery(document).ready(function() {
  ...
  if (
    jQuery("#singleClinicTemplate").length ||
    jQuery("#clinicasIolUrl").length
  ) {
    //loadGmapsScript();
    gmapsClinicsLoader();
  }
...
function gmapsClinicsLoader() {
  ...
  if (jQuery("#clinicasIolUrl").length) {
    ...
    jQuery("#triggerUbicacion").on("click", function() {
      ...
      if (stringLocation == "") {
        alert(
          "Ha de introducir al menos algo relacionado con la dirección de su ubicación actual"
        );
        return false;
      }
  ...
  geocoder.geocode({ latLng: latlng }, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      ...
      */
        ...
        jQuery("#userGeoInfoSpanValue,span.userGeoInfoSpanValue").html(
          results[1].formatted_address
        );

        jQuery("#distanceValue,span.distanceValue ").html(
          jQuery("#comboboxDistanceToClinic").val()
        );
      } else {
        jQuery("#userGeoInfoSpanValue, span.userGeoInfoSpanValue").html(
...
function getWithinDistanceClinicsCoordsAddMap(forzar) {
  ...
  if (!userLatitude || !userLongitude || forzar) {
    ...
    qStringACW =
      "action=getClinicsCoordsWithinDistance&distanceToClinic=" +
      distanceToClinic +
      "&userLatitude=" +
      getLatitude() +
      "&userLongitude=" +
...
function createMapAddMarquersToGeoOrNot(clinicas, cLatitude, cLongitude) {
  ...
  //Añadiremos la lista de clínicas en dos sitios distintos en función de que estemos en el búscador o no.
  if (jQuery("#content.template-bucador-clinicas").length) {
    jQuery(".resultLCom").fadeOut("600");
    jQuery("#rightBuscadorClinicas div.userGeoInfo").fadeIn("600");
    jQuery("#geoCond").empty();
    var list = jQuery("#geoCond")
      .append("<ul></ul>")
      .find("ul");
  } else {
    var list = jQuery(".geoSelectedList")
      .append("<ul></ul>")
      .find("ul");
  }
  ...
  for (var clinica in clinicas) {
    ...
    if (clinicas[clinica].apMaps != -1) {
      list.append(
        '<li><a href="' + clinicas[clinica].link + '">' + clinica + "</a></li>"
      );
    }
    // continue;
    /*  }*/

    clinicaCoord[clinica] = new google.maps.LatLng(
      clinicas[clinica].latitud,
      clinicas[clinica].longitud
    );
...
function getUserCoordsGetClinicasWithinCreateMap(distanceToClinic) {
  console.log(
    "getUserCoordsGetClinicasWithinCreateMap ejecutada por primera vez"
  );

  if (
    !userLatitude ||
    !userLongitude ||
    userLatitude == directionTextLatitude ||
    userLongitude == directionTextLongitude
  ) {
      ...
      function(objError) {
        ...
        switch (objError.code) {
          ...
          case objError.POSITION_UNAVAILABLE:
            divMapa.innerHTML =
              "La información de su posición no está disponible.";
            break;
...
function createClinicMap(cLatitude, cLongitude, title, user) {
  ...
  if (!jQuery("#singleClinicCoordChilds").length) {
    ...
    var objCoordenadasUser = new google.maps.LatLng(
      getLatitude(),
      getLongitude()
    ); //userLatitude,userLongitude
    //opciones del mapa
    ...
    for (var cObject in jsonCoords) {
      ...
      var objCoordenadasRefClinic = new google.maps.LatLng(
        jsonCoords[cObject]["lat"],
        jsonCoords[cObject]["long"]
      );
...
function calcRouteToClinic() {
  ...
  */
  map = new google.maps.Map(
    document.getElementById("divSingleClinicMapa"),
    mapOptions
  );
