...
function clinicFormComboButtonLoader() {
  ...
  jQuery("#clinica_filter_form #comboboxViewType").combobox({
    select: function(event, ui) {
      ...
      var newUrlAjaxQuery = UpdateQueryString(
        "viewType",
        viewTypeValue,
        urlAjaxQuery
      );

      var newAjaxQuery = newUrlAjaxQuery.substring(
        newUrlAjaxQuery.lastIndexOf("?") + 1,
        newUrlAjaxQuery.length
      );
...
function clinicFormAccordionLoader() {
  ...
  jQuery("#accordionFilterSimple,#accordionFilterSecond").each(function() {
    ...
    jQuery(thisHandlerid).on("click", function() {
      return false;
    });

    var thisReseterId = "#reseter-" + jQuery(this).attr("id");
    jQuery(thisReseterId).on("click", function() {
      return false;
    });
    ...
    jQuery(this).draggable({
      ...
      stop: function(event, ui) {
        ...
        jQuery(this)
          .css("position", "fixed")
          .css("top", topElement - docScroll)
          .css("left", leftElement);
      }
    });
  });
}
...
function clinicaQueryStringUpdater() {
  if (
    ...
    jQuery("#currentQueryString").text(
      the_ajax_script.ajaxurl + "?" + urlQString
    );
...
function PannelClinicaClickResponse() {
  ...
  var newAlturaContenidoPannel =
    parseInt(
      jQuery("#expanderContent")
        .css("height")
        .replace("px", "")
    ) + 35;
  var newAnchuraContenidoPannel =
    parseInt(
      jQuery("#expanderContent")
        .css("width")
        .replace("px", "")
  ...
  switch (clinicaClickeado) {
    case 0:
      ...
      jQuery(".resizable").resizable(
        "option",
        "minHeight",
        newAlturaContenidoPannel
      );
      jQuery(".resizable").resizable(
        "option",
        "minWidth",
        newAnchuraContenidoPannel
      );
      jQuery("#expanderSign")
...
function ifClinicaCollapsedExpand() {
  if (clinicaClickeado == 0) {
    ...
    var newAlturaContenidoPannel =
      jQuery("#expanderContent li").length * 20 + 80;
  ...
  jQuery.ajax({
    ...
    success: function(response_from_the_action_function) {
      ...
      if (typeof viewTypeData === "undefined") {
        ...
        var textoInfoPannel = jQuery(
          "h4#expanderHead span.infoIolHeaderTitle"
        ).html();

        jQuery("h4#expanderHead span.infoIolHeaderTitle")
          .css("color", "#FEA63C")
          .text("Ver Información de la búsqueda"); //textoInfoPannel
        switch (Clickeado) {
          case 0:
...
function loadGmapsScript() {
  jQuery.getScript(
    "https://maps.google.com/maps/api/js?key=AIzaSyDJ3FwVMCRgviEBGXzFmVP5JblEEyieqz8" //&sensor=false
  );
  jQuery.getScript(
    "https://www.nuevocristalino.es/wp-content/plugins/clinica/js/clinica-maps.js"
  );
}
