function archive_submit_me(viewTypeData){
    ...
    if (typeof(viewTypeData) === 'undefined' || 1==1 ) {
        ...
        if (jQuery('#modelosIol').length && jQuery('#archiveUrl').length) { //jQuery('#archiveUrl').length && jQuery('#modelosUrl').length
            ...
            var href = archive_page + '?pt=no&' + jQuery('#iol_filter_form').serialize();//añadimos lo del pt=no
...
if (typeof history.pushState !== "undefined") {
    ...
    jQuery(selectorsStringScroll).live('click', function (){
        var href = jQuery(this).attr('href');
    ...
    //A los links de abajo y a los de #content-destacado le damos el scroll to top.
     jQuery('#destacados .destacado1 h2 a, #mVisitedIols a, #mVisitedClinics a, #corporativeLinks a, div.link-to-archive a, .content-destacado a, .block-last h2 a, .block h2 a, block-last h2 a').live('click', function(){
...
function getForm(action,qString) {
	...
	data = qString+'&action='+action+textPostId;//get_adv_iol_filter_form
    data = data.replace('?','');
    jQuery('#right form, #right H3').fadeTo('fast', 0.5);//'#right
    ...
    jQuery.ajax({
     ...
     //   cache: false,
        success: function (response) {
            ...
            //De la función respuesta tambien cogeremos #main -> En un segundo paso esto no será necesario puesto que será lo único que devolvamos con ajax.
            console.log('Petición Ajax realizada con éxito');
            var mainContent = response;//jQuery(response).find('#right');
                        
            jQuery('#right').html(mainContent);//#right
...
if(jQuery('#changeFilter').length){
    ...
    jQuery('#changeFilter a').each(function () { //jQuery('#changeFilter button').each(function(){
                ...
                jQuery('#menu-menu-site a').each(function () { //not('#footer-wrap a, #top-header a')
                    ...
                    if (hrefA != undefined) {
                        if (hrefA.indexOf("pt=yes") != -1) {
                            ...
                            hrefA = hrefA + "?pt=no"; //No añadimos nada.
                        }
                        jQuery(this).attr('href', hrefA);
                    }
                });
