...
function PatientFormLoader(){
	...
	//Hacemos lo mismo para el resto de inputs.
	 ...
	 //Metemos la Cookie de que es un paciente y llamamos a la función que hace los cambios pertinentes
	 //Mañana se cambia
...
/* Como no se haya llamado la función surgeryShowResultsLoader() antes de mostrar los resultados de satisfacción, da error. */
 ...
 jQuery(document).ready(function (){
 	...
 	if((jQuery('#templatePostOpTestResult').length) || (jQuery('.templateUdaDisplayData').length) ){ //jQuery('#templateUdaDisplayData').length
 	  //alert('reconocida');
     ...
     jQuery("#satIol, #dDriving, #nDriving, #iVision, #newspaper, #prices, #needle, #dGlasses, #nGlasses, #currentVision").tabs( //#dDriving
       ...
       create:
                      ...
                      if(jQuery(window).width() <= 640){
                        ...
                        console.log('Intentando charts con: '+mobileWidth);
                            var options = {
                                            title: titleLeyenda,
                                            width: mobileWidth,
                                            height: 300
 ...
 /*-- No vamos a hacerlo en js propio--*/

  if(jQuery('.templateUdaDisplayData').length){ //jQuery('#templateUdaDisplayData').length

  	var idsToBeTabbed = '';
  	idsToBeTabbedArray= [];
  	if(jQuery('.idsToBeTabbed').length>1){

 jQuery('.idsToBeTabbed').each(function(){
 			      idsToBeTabbedArray.push(jQuery(this).data('idstobettabed'));
 	});

  idsToBeTabbed = idsToBeTabbedArray.join(',');

			  	//idsToBeTabbed = jQuery('.idsToBeTabbed').data('idstobettabed'); //jQuery('#idsToBeTabbed').data('idstobettabed');

		}else{
					idsToBeTabbed = jQuery('.idsToBeTabbed').data('idstobettabed'); //jQuery('#idsToBeTabbed').data('idstobettabed');
	}

		console.log('Los ids to be tabbed son: '+idsToBeTabbed);

     jQuery(idsToBeTabbed).tabs( //#dDriving
       {
       active: 1,
       //beforeLoad: function(event,ui){alert('beforeLoad triggered')},
       //Inicio del Create
       create:
       	       	function (event, ui) {
       	       	    ...
       	       	    var str = '#tabs' + numberQ + '-'; //+numberQ;
       	       	    //console.log(str);
       	       	    var filterUserMetaDataValue = selector.replace(str, '');
       	       	    //console.log(surgeryIol);
   	    						var filterUserMetaDataKey = jQuery(selector).data("udafilterkey");
   	    						var actualUserDataValueId = jQuery(selector).data("actualuserdatavalueid");
										var dataudisplaynottintoaccount = JSON.stringify(jQuery(selector).data("dataudisplaynottintoaccount"));
										var userskeyvalueexcluded = JSON.stringify(jQuery(selector).data("userskeyvalueexcluded"));
										var userskeyvalueincludedonly = JSON.stringify(jQuery(selector).data("userskeyvalueincludedonly"));

										//alert(dataudisplaynottintoaccount);

       	       	    var uMetaDataKey = jQuery(this).attr('id');
       	       	    //alert(question);
       	       	    console.log('a continuación el Ajax');
       	       	    jQuery.ajax({
       	       	        url: the_ajax_script.ajaxurl,
       	       	        data: 'action=getUserMetadata&uMetaDataKey=' + uMetaDataKey + '&filterUserMetaDataKey='+filterUserMetaDataKey+'&filterUserMetaDataValue=' + filterUserMetaDataValue +'&filterUserMetaDataValueId='+actualUserDataValueId+'&dataudisplaynottintoaccount='+dataudisplaynottintoaccount+'&userskeyvalueexcluded='+userskeyvalueexcluded+'&userskeyvalueincludedonly='+userskeyvalueincludedonly,
       	       	        cache: true,
       	       	        beforeSend: function () {
       	       	            jQuery('#ajaxResult' + numberQ + '-' + filterUserMetaDataValue).hide();
                            console.log('Mostrando #loading' + numberQ + '-' + filterUserMetaDataValue + ' img');
                            jQuery('#loading' + numberQ + '-' + filterUserMetaDataValue + ' img').show();


       	       	        },
       	       	        complete: function () {
       	       	            jQuery('#ajaxResult' + numberQ + '-' + filterUserMetaDataValue).show();
       	       	            jQuery('#loading' + numberQ + '-' + filterUserMetaDataValue + ' img').hide();
       	       	        },
       	       	        success:
        					function (response_from_the_action_function) {
        					    jQuery('#loading' + numberQ + '-' + uMetaDataKey + ' img').hide();
        					    var title = 0;
        					    jQuery('#ajaxResult' + numberQ + '-' + filterUserMetaDataValue).html(response_from_the_action_function);
        					    //alert('#ajaxResult'+numberQ+'-'+surgeryIol);
        					    var data = new google.visualization.DataTable();
        					    var pregunta = response_from_the_action_function[0][0];
        					    var tipo_Iol = response_from_the_action_function[0][1];
...
//Vamos a cambiar los valores anteriores para ver si se pueden adaptar a móviles.
  ...
  console.log('Intentando charts con: '+mobileWidth);
      var options = {
                      ...
                      height: 300
        					    ...
        					    var chart = new google.visualization.PieChart(document.getElementById('ajaxResult' + numberQ + '-' + filterUserMetaDataValue));
...
jQuery(document).ready(function () {
    ...
    if (jQuery('#sliderMBloqsWrapper #slider').length) {
        ...
        if(jQuery('#bloque-cirujano').length){

        	if(jQuery('body #fakeDivDown').length== 0){
           jQuery('body').append('<div id="fakeDivDown">&nbsp;</div>');
        		}
...
function fakeDivGenerator() {
    ...
    var topP = topGRef.top + pTopGRef.top - 160;//185,,160,,20
...
if(jQuery('#interaction-bloq').length){
	...
	if(jQuery(window).width() >= 640){
	jQuery('body #fakeDivDown').css({ 'position': 'absolute',
    'display':'block',
    'height':  '360px', //heigh +
    'width': '100%', //width
